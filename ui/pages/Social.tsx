...
export const Social: React.FC = () => {
  ...
  // STRICT SEARCH LOGIC: Only show results if there is a query, and only if it matches from the START
  const filteredDiscovery = useMemo(() => {
    const query = userQuery.trim().toLowerCase();
    if (!allUsers || query.length === 0) return [];
    
    return allUsers.filter(u => {
      // Don't show myself or existing friends
      if (u?.id === currentUser?.id || friends?.some(f => f.id === u.id)) return false;

      // Strict Prefix Match: Must start with the query
      const matchesUsername = u?.username?.toLowerCase().startsWith(query);
      const matchesFullName = u?.name?.toLowerCase().startsWith(query);
      const matchesFirstName = u?.firstName?.toLowerCase().startsWith(query);
      const matchesLastName = u?.lastName?.toLowerCase().startsWith(query);

      return matchesUsername || matchesFullName || matchesFirstName || matchesLastName;
    });
  ...
  return (
    <div ref={scrollContainerRef} className="space-y-10 animate-in fade-in duration-500 overflow-anchor-none">
          ...
          <section className="bg-indigo-600 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="space-y-4">
                {activeChallenges.map(challenge => {
                    ...
                    const getIcon = () => challenge.type === 'BRACKET' ? '🥊' : (challenge.type === 'TIERLIST' ? '📊' : '🕵️');
            ...
            {pendingRequests.map(req => (
              <div key={req.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-amber-100 dark:border-amber-900/20 flex items-center justify-between shadow-sm">
                ...
                <div className="flex gap-2">
                  <button onClick={() => currentUser && acceptRequest(currentUser.id, req.id)} className="px-4 py-2 bg-indigo-600 text-white text-xs font-black rounded-xl hover:bg-indigo-700 transition">Accept</button>
                  <button onClick={() => currentUser && rejectRequest(currentUser.id, req.id)} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-black rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600 transition">Ignore</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Discovery Section - Empty by default */}
              ...
              return (
                <div key={user.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow">
                  ...
                  {isRequested ? (
                    ...
                    <button onClick={() => currentUser && sendRequest(currentUser.id, user.id)} disabled={isProcessing} className="px-4 py-2 bg-indigo-600 text-white text-[10px] font-black uppercase rounded-xl hover:bg-indigo-700 transition disabled:opacity-50">
